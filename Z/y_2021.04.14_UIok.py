...
#MY_ROOT_PATH = 'C:\\Users\\Y32840\\Desktop\\3DScanDataConverter\\Scripts'

# Add the root path to the environment
sys.path.insert(1, MY_ROOT_PATH)
...
class UIApp:
    ...
    def __init__(self, root):
        ...
        #setting icon
        root.iconbitmap(os.path.join(MY_ROOT_PATH, 'Icons\\icon.ico'))
        #setting window size
        width=500
        height=400
        ...
        GButton_878.place(x=336,y=350,width=50,height=40)
        GButton_878["command"] = self.GButton_878_command


        GButton_879=tk.Button(root)
        GButton_879["activebackground"] = "#999999"
        GButton_879["activeforeground"] = "#000000"
        GButton_879["bg"] = "#efefef"
        ft = tkFont.Font(family=my_font,size=13)
        GButton_879["font"] = ft
        GButton_879["fg"] = "#000000"
        GButton_879["justify"] = "center"
        GButton_879["text"] = TEXT["Data_Converter"][language]
        GButton_879["relief"] = "raised"
        GButton_879.place(x=386,y=350,width=50,height=40)
    ...
    def GButton_878_command(self, *args):
        ...
        for file_path in input_files:
            ...
            try:
                p = Popen("run_converter2.bat", cwd=r"{}".format(MY_ROOT_PATH))
                stdout, stderr = p.communicate()
            except:
                tk.messagebox.showinfo('Error', '再起動して')


            #batchFileLocation = ''
            #batchFileFullPath = os.path.join(batchFileLocation, 'run_converter2.bat')
            #print(batchFileFullPath)
            #p = subprocess.Popen('run_converter2.bat', shell=True, stdin=subprocess.PIPE)
            #stdout,stderr = p.communicate()

            # subprocess.call([r'run_converter2.bat'])

            #subprocess.run(["Python3.6.6\python converter2.py"],stdout = subprocess.PIPE, stderr = subprocess.PIPE,shell=True)
...
##############################################
    def GButton_879_command(self, *args):
    ...
    #cm1 = MY_ROOT_PATH + '\\' + "Python3.6.6\\python "
    #cm1 += (MY_ROOT_PATH + '\\' + "converter2.py")
    #print(cm1)
    #cm2 = "Python3.6.6\\python converter2.py"
