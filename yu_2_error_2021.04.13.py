...
def mesh_filter(file_path):
    ...
    # Close vertices
    ms.merge_close_vertices(threshold=0.1)
    ...
    try:
        print('surface_reconstruction_screened_poisson preclean=False')
        ms.surface_reconstruction_screened_poisson(
            visiblelayer=False, depth=my_settings['poisson_depth'], fulldepth=5, cgdepth=0, scale=1.1, samplespernode=1.5, pointweight=4,
            iters=8, confidence=False, preclean=False)     # preclean= False
    except:
        print('surface_reconstruction_screened_poisson preclean=True')
        ms.surface_reconstruction_screened_poisson(
            visiblelayer=False, depth=my_settings['poisson_depth'], fulldepth=5, cgdepth=0, scale=1.1, samplespernode=1.5, pointweight=4,
            iters=8, confidence=False, preclean=True)     # preclean= False
    
    # Reduce face numble
    ms.simplification_quadric_edge_collapse_decimation(targetfacenum=my_settings['targetfacenum'], targetperc=0, qualitythr=0.3,
                                                       preserveboundary=False, boundaryweight=1, preservenormal=False, preservetopology=True,
...
def convert_stl_iges(file_path):
    ...
    print('Read settings FreeCad' + settings_file_path)

    my_print('Convert Data')

    my_settings = read_my_settings(settings_file_path)
...
##############################
def read_my_settings(settings_file_path=''):
...
Setting
{"FreeCAD_path": "C:\\Program Files\\FreeCAD 0.18\\bin", "hc_laplacian_smooth_time": 1, "poisson_depth": 8, "targetfacenum": 8000, "shape_tolerance": 0.1, "export_file_type": ["iges", "step"], "input_path": "C:\\Users\\Y32840\\Desktop\\ScanDataConverter\\CAD_DATA\\InputFile", "output_path": "C:\\Users\\Y32840\\Desktop\\ScanDataConverter\\CAD_DATA\\OutputFile", "processing_path": "", "language": "JA"}
    ...
    def GButton_878_command(self, *args):
        ...
        for file_path in input_files:
            ...
            try:
                ...
                tk.messagebox.showinfo('Error', 'Meshlab Error')
